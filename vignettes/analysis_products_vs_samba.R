...
if (cs == "npw") {
  ...
  samb <- terra::rast("./input/samba/npw_inference_predmean_2021010106_2021013105.tif")
  out_dir <- "./output/npw/"
  phi_nyc <- cities[which(cities$ST %in% c("NJ", "NY", "PA")), ]
  cities <- phi_nyc[which(!(phi_nyc$NAME %in% c(
    "Buffalo",
    "Pittsburgh",
    "Erie",
    "Rochester",
  ...
  samb <- terra::rast("./input/samba/nps_inference_predmean_2024070105_2024073104.tif")
  out_dir <- "./output/nps/"
  phi_nyc <- cities[which(cities$ST %in% c("NJ", "NY", "PA")), ]
  cities <- phi_nyc[which(!(phi_nyc$NAME %in% c(
    "Buffalo",
  ...
  samb <- terra::rast("./input/samba/phoe_inference_predmean_2023070109_2023073123.tif")
  out_dir <- "./output/phoe/"
  ...
  samb <- terra::rast("./input/samba/tri_inference_predmean_2021070105_2021073123.tif")
  out_dir <- "./output/tri/"
...
for (d in seq_along(dates)) {
  ...
  timeserie <- terra::global(samb[[idx]],
  ...
  tn <- floor(
    min(terra::minmax(samb[[idx_plot_hly]])[1],
        terra::minmax(dm_tmin[[idx_plot_dly]])[1],
        terra::minmax(gm_tmmn[[idx_plot_dly]])[1])
    )
  tx <- ceiling(
    max(terra::minmax(samb[[idx_plot_hly]])[2],
...
# TN avg
samb_tn <- samb[[which(terra::time(samb) %in% as.POSIXct(unlist(hours_tn), tz = tz))]]
mean_samb_tn <- terra::mean(samb_tn)
mean_gm_tmmn <- terra::mean(gm_tmmn)
mean_dm_tmin <- terra::mean(dm_tmin)
tn <- floor(
  min(
    terra::minmax(mean_samb_tn)[1],
...
tx <- ceiling(
  max(
    terra::minmax(mean_samb_tn)[2],
    terra::minmax(mean_dm_tmin)[2],
    terra::minmax(mean_gm_tmmn)[2]
  )
)
p1 <- map_temp(mean_samb_tn, borders = cities, tn = tn, tx = tx, xlim = xlim, ylim = ylim) +
  labs(caption = paste0(
    "Source: BHM T2M, min")
  )
p2 <- map_temp(mean_dm_tmin, borders = cities, tn = tn, tx = tx, xlim = xlim, ylim = ylim) +
  labs(caption = paste0("Source: tmin daymet\n"))
p3 <- map_temp(mean_gm_tmmn, borders = cities, tn = tn, tx = tx, xlim = xlim, ylim = ylim) +
  labs(caption = paste0("Source: tmmn gridMET\n"))
all <- ggpubr::ggarrange(p3,
                         p2,
                         p1,
                         nrow = 1,
                         common.legend = TRUE,
...
for (d in seq_along(dates)) {
  ...
  dm_tmax <- dm_tmax |>
    terra::project("epsg:4326")
  p1 <- map_temp(samb[[idx_plot_hly]], borders = cities, tn = tn, tx = tx, xlim = xlim, ylim = ylim) +
    labs(caption = paste0(
    "Source: BHM T2M, max measured at\n",
    lubridate::with_tz(ts_plot, tz),
    " (local time)")
    )
  p2 <- map_temp(dm_tmax[[idx_plot_dly]], borders = cities, tn = tn, tx = tx, xlim = xlim, ylim = ylim) +
    labs(caption = paste0("Source: tmax daymet\n", dates[d]))
  p3 <- map_temp(gm_tmmx[[idx_plot_dly]], borders = cities, tn = tn, tx = tx, xlim = xlim, ylim = ylim) +
    labs(caption = paste0("Source: tmmx gridMET\n", dates[d]))
  all <- ggpubr::ggarrange(p3,
                           p2,
                           p1,
                           nrow = 1,
...
# tx avg
samb_tx <- samb[[which(terra::time(samb) %in% as.POSIXct(unlist(hours_tx), tz = tz))]]
mean_samb_tx <- terra::mean(samb_tx)
mean_gm_tmmx <- terra::mean(gm_tmmx)
mean_dm_tmax <- terra::mean(dm_tmax)
tn <- floor(
  min(
    terra::minmax(mean_samb_tx)[1],
...
tx <- ceiling(
  max(
    terra::minmax(mean_samb_tx)[2],
    terra::minmax(mean_dm_tmax)[2],
    terra::minmax(mean_gm_tmmx)[2]
  )
)
p1 <- map_temp(mean_samb_tx, borders = cities, tn = tn, tx = tx, xlim = xlim, ylim = ylim) +
  labs(caption = paste0(
    "Source: BHM T2M, max")
  )
p2 <- map_temp(mean_dm_tmax, borders = cities, tn = tn, tx = tx, xlim = xlim, ylim = ylim) +
  labs(caption = paste0("Source: tmax daymet\n"))
p3 <- map_temp(mean_gm_tmmx, borders = cities, tn = tn, tx = tx, xlim = xlim, ylim = ylim) +
  labs(caption = paste0("Source: tmmx gridMET\n"))
all <- ggpubr::ggarrange(p3,
                         p2,
                         p1,
...
samb_pts_tn <- terra::as.points(mean_samb_tn)
...
p_diff_dm_tn <- map_temp(diff_dm_tn, borders = cities, tn = -10, tx = 10, xlim = xlim, ylim = ylim) +
  labs(title = "daymet - samba") +
  scale_fill_stepsn(
...
p_diff_gm_tn <- map_temp(diff_gm_tn, borders = cities, tn = -10, tx = 10, xlim = xlim, ylim = ylim) +
  labs(title = "gridmet - samba") +
  scale_fill_stepsn(
    colours = c("blue", "#F7F7F7", "#D6604D"),
    na.value = NA,
    limits = c(-10, 10),
...
all_tn_diff
ggsave(plot = all_tn_diff,
       file = paste0(out_dir, "products_vs_samba_tn_avg_diff.png"),
       bg = "white",
       height = 6,
       width = 12,
       dpi = 300)

samb_pts_tx <- terra::as.points(mean_samb_tx)
...
p_diff_dm_tx <- map_temp(diff_dm_tx, borders = cities, tn = -10, tx = 10, xlim = xlim, ylim = ylim) +
  labs(title = "daymet - samba") +
  scale_fill_stepsn(
    colours = c("blue", "#F7F7F7", "#D6604D"),
    na.value = NA,
...
p_diff_gm_tx <- map_temp(diff_gm_tx, borders = cities, tn = -10, tx = 10, xlim = xlim, ylim = ylim) +
  labs(title = "gridmet - samba") +
  scale_fill_stepsn(
    colours = c("blue", "#F7F7F7", "#D6604D"),
    na.value = NA,
...
all_tx_diff
ggsave(plot = all_tx_diff,
       file = paste0(out_dir, "products_vs_samba_tx_avg_diff.png"),
       bg = "white",
       height = 6,
       width = 12,
       dpi = 300)
