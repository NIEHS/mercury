...
#' @import terra
#' @import tidyterra
map_temp_products_city <- function(city,
                                   hw_dict,
                                   gridmet_dir,
                                   daymet_dir,
                                   moment_of_day) {
  x <- load_temp_products_city(city,
                               hw_dict,
                               gridmet_dir,
                               daymet_dir,
                               moment_of_day)
  ...
  mint <- floor(min(terra::minmax(x$gridmet)[1],
              terra::minmax(x$daymet)[1],
              terra::minmax(x$heatwatch_r)[1]))
  maxt <- ceiling(max(terra::minmax(x$gridmet)[2],
              terra::minmax(x$daymet)[2],
              terra::minmax(x$heatwatch_r)[2]))
  map_hw <- map_temp_heatwatch(r = x$heatwatch_r,
                               tr = x$heatwatch_t,
                               borders = poly,
                               temp_unit = "C") +
